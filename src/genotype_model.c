...
static void calc_gt_prob(gt_meth *gt, sr_param *param, char rf) {
  double e = exp((float)gt->aq * -0.1 * LOG10);
  double l = 1.0 - param->under_conv;
  double t = param->over_conv;
  ...
  double ll[11];
  double ref_bias = -log(param->ref_bias);
  // Add in prior from reference
  ...
  gt->gt_gof = ll[0] > max ? (ll[0] - max) / LOG10 : 0.0;
}
#endif
