						...
						for(uint64_t ix = start_idx; ix < read_idx; ix++) {
							...
							if(al->forward_position == al1->forward_position && al->reverse_position == al1->reverse_position && al->bs_strand == al1->bs_strand) {
								...
								if((maxq < maxq1) || (maxq == maxq1 && get_al_qual(al) < get_al_qual(al1))) {
									align_details *thash;
									HASH_FIND(hh, align_hash, gt_string_get_string(al1->tag), gt_string_get_length(al1->tag), thash);
									if(thash != NULL) HASH_DEL(align_hash, thash);
									al_p[ix] = al;
									al->idx = ix;
									HASH_FIND(hh, align_hash, gt_string_get_string(al->tag), gt_string_get_length(al->tag), thash);
				...
				if(pos == curr_pos) {
					...
					for(uint64_t ix = start_idx; ix < read_idx; ix++) {
						...
						if(al->forward_position == al1->forward_position && al->reverse_position == al1->reverse_position &&
								al->bs_strand == al1->bs_strand && ((al1->alignment_flag & 9) == 9 || ((al1->alignment_flag & 9) == 0))) {
...
static void init_stats(sr_param *par) {
	...
	if(par->json_file == NULL) fprintf(stderr,"Could not open report file '%s' for output: %s\n", par->report_file, strerror(errno));
		...
		par->stats = calloc((size_t)1, sizeof(bs_stats));
...
gt_status parse_arguments(int argc, char **argv) {
	...
	while (true) {
		...
		case 'c':
			...
			if(param.under_conv < 0.0 || param.under_conv > 1.0) param.under_conv = DEFAULT_UNDER_CONVERSION;
			if(param.over_conv < 0.0 || param.over_conv > 1.0) param.over_conv = DEFAULT_OVER_CONVERSION;
